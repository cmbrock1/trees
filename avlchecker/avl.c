...
compare(char *s1,char *s2)
    ...
    if (s1[s1l-1] == '+' || s1[s1l-1] == '-')
         {
         s1c = s1[s1l-1];
         s1[s1l-1] = '\0';
         }

    if (s2[s2l-1] == '+' || s2[s2l-1] == '-')
         {
         s2c = s2[s2l-1];
         s2[s2l-1] = '\0';
         }

    printf("really comparing %s and %s\n",s1,s2);
    result = strcmp(s1,s2);

    if (s1c != 0) s1[s1l-1] = s1c;
    if (s2c != 0) s2[s2l-1] = s2c;
    printf("done comparing %s and %s\n",s1,s2);
...
checkOrder(tnode *t)
    ...
    if (t->left != 0)
        {
        if (compare(t->left->name,t->name) >= 0)
    ...
    if (t->right != 0)
        {
        if (compare(t->right->name,t->name) <= 0)
